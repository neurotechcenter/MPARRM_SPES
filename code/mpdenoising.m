...
nbpnt   = size(data,2);
nbtrial = size(data,3);
mpDataSel = nan(size(data));  % selected signal
mpDataRsd = nan(size(data));  % residue signal
...
if ~exist(platformSpecificNameMPP([g.mpPath '/' mpNameType '/GaborMP/mp0.bok.000']),'file')
...
%% retrive the data
if g.calRetrive
    % check the retrive signal
    calRetrive = true;
    if exist([g.mpPath '/' mpNameType '/retrive_' num2str(g.mpIteration) '.mat'],'file')==2
        load([g.mpPath '/' mpNameType '/retrive_' num2str(g.mpIteration) '.mat'],'mpDataSel','gRetr');
        if g.srate == gRetr.srate && g.mpCenterLoc == gRetr.mpCenterLoc && g.mpIteration == gRetr.mpIteration && ...
           g.mpWinLen == gRetr.mpWinLen && g.mpFreThre == gRetr.mpFreThre && ~g.figShow
           calRetrive = false;
        end
    end
    ...
    % retrive the signal
    if calRetrive
        % load the Matching Pursuit parameter
        disp('calRetrive...');
        ...
        for s  = 1:nbtrial
            ...
            % retrive the data
            datSel     = zeros(1,mp.L);
            atomSelect = atomSelect(1:g.mpIteration);
        ...
        gRetr = g;
        save([g.mpPath '/' mpNameType '/retrive_' num2str(g.mpIteration) '.mat'],'mpDataSel','gRetr');
    end
end
